...
def post_process(pdf_file: Path, context: PdfContext, executor: Executor) -> Path:
	...
	if context.options.output_type.startswith('pdfa'):
		if context.options.no_graft:
			log.warning('Since text grafting is off, conversion to PDF/A is ignored!! PDF/A conversion may result in non-compatible encoding.')
		else:
	...
	def update_page(result: PageResult, pbar):
		try:
			...
			ocrgraft.graft_page(
				...
				image=image, #result.ocr,# result.pdf_page_from_image, 
				textpdf=textpdf, #result.ocr, #
