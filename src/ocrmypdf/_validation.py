...
def check_options_output(options):
	...
	if not is_latin and  options.pdf_renderer.startswith('hocr'):
		if options.fontfile is None or not options.no_graft:
			log.warning(
				"The 'hocr' PDF renderer is known to cause problems with one "
				"or more of the languages in your document.  Use either"
				"`--pdf-renderer auto` (the default) and `--ocr-engine tesseract` to avoid issues.\n"
				"Alternatively, use `--no-graft` along with a valid font that can handle non-latin text (see `--fontfile` and `--fontname` args.)"
			)

	if options.no_graft:
		log.warning('Some options will not work properly when `--no-graft` is set. '
		'These include conversion to PDF/A and page rotation.')
